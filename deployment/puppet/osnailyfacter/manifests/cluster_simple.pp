  ...
  case $::fuel_settings['role'] {
    ...
    "compute" : {
      ...
      if $monitoring_hash['odc'] {
        class {'odc':
          username        =>      'nova',
          password        =>      $::osnailyfacter::cluster_simple::nova_hash[user_password],
          tenant_name     =>      'services',
          auth_url        =>      '127.0.0.1:35357/v2.0',
          token           =>      $::osnailyfacter::cluster_simple::keystone_hash[admin_token],
    ...
    if $monitoring_hash['monitoring_server'] == 'nagios' {
        # for completeness we should include "rabbit" and "mysql" but there are some issues with the nrpe to be explored
              class {'nagios::master':
                      ...
                      contacts        => {'email' => $monitoring_hash['nagios_email'], 'user' => 'admin', 'alias' => 'Nagios Admin', 'group' => 'admins'},
